...
function formatPhotonFeature(feat: any): LivePOI {
  ...
  if (osmVal.includes("cafe") || nameLower.includes("cafe") || nameLower.includes("cà phê") || nameLower.includes("tea")) {
    category = "cafe";
    ...
    icon = "🍜";
  } else if (osmKey === "shop" || osmVal.includes("shop") || osmVal.includes("supermarket") || osmVal.includes("convenience")) {
    category = "shop";
...
// 1. Tìm kiếm quán xá, số nhà, địa chỉ theo từ khóa qua API Photon (OSM)
...
// 2. Tải danh sách quán ăn, cafe, cửa hàng, số nhà thực tế quanh vị trí tâm bản đồ
export async function fetchNearbyPOIsAPI(
  ...
  try {
    const promises = queries.map((q) =>
      fetch(`https://photon.komoot.io/api/?q=${encodeURIComponent(q)}&lat=${lat}&lon=${lng}&limit=8`).then(
        (r) => (r.ok ? r.json() : { features: [] })
      )
    );
    ...
    return combinedFeatures.map(formatPhotonFeature);
  } catch (err) {
