...
async def get_eco_locations(
    ...
    try:
        ...
        print("[EcoLocations] Lỗi truy vấn incidents:", e)
    ...
    try:
        ...
        for row in green_rows.mappings():
            ...
            locations.append({
                ...
                "metricLabel": "Quy mô diện tích",
                "metricValue": meta.get("area", "Khuôn viên sinh thái"),
                "description": f"{row['name']} là mảng xanh đô thị quan trọng, cung cấp không khí trong lành và bảo tồn đa dạng sinh học.",
                "rating": float(meta.get("rating", 4.8)),
            })
    except Exception as e:
        print("[EcoLocations] Lỗi truy vấn essential_facilities:", e)

    # 3. Truy vấn Trạm thu gom rác tái chế & E-waste
    recycling_query = """
        SELECT 
    ...
    """
    try:
        recycling_rows = await db.execute(text(recycling_query))
        for row in recycling_rows.mappings():
            accepted = row["accepted_waste_types"] or []
            accepted_str = ", ".join(accepted) if isinstance(accepted, list) else str(accepted)
            locations.append({
                "id": f"rec-{row['id']}",
                "name": row["name"],
                "category": "recycling",
                ...
                "longitude": row["longitude"],
                "status": "active",
                "statusText": "Đang tiếp nhận rác phân loại",
                "metricLabel": "Loại rác tiếp nhận",
                "metricValue": accepted_str or "Pin, Nhựa, Giấy, Kim loại",
                "description": f"{row['name']} tiếp nhận thu gom và tái chế: {accepted_str}. Giờ mở cửa: {row['operating_hours'] or '08:00 - 17:00'}.",
                "acceptedTypes": accepted_str,
                "contactPhone": row["contact_phone"] or "1900 1080",
                "managingOrg": row["managing_organization"] or "Sở Tài nguyên & Môi trường",
            })
    except Exception as e:
        print("[EcoLocations] Lỗi truy vấn recycling_facilities:", e)

    # 4. Truy vấn Trạm quan trắc IoT viễn trắc
    iot_query = """
        SELECT 
    ...
    try:
        ...
        print("[EcoLocations] Lỗi truy vấn iot_sensor_stations:", e)

    # 5. Truy vấn Điểm đen ngập lụt & Triều cường đô thị
    ...
    try:
        ...
        print("[EcoLocations] flood_hotspots db query note:", e)

    # Fallback to HCMC_FLOOD_HOTSPOTS nếu bảng flood_hotspots chưa nạp dữ liệu
    if not flood_loaded:
        for spot in HCMC_FLOOD_HOTSPOTS:
            hist_d = spot["historical_depth_cm"]
            depth_cm = int(hist_d * (0.6 if rain_mmh > 20 else 0.2))
            severity = "SEVERE" if depth_cm > 35 else "MODERATE" if depth_cm > 15 else "MINOR" if depth_cm > 5 else "NONE"
            loc_status = "warning" if depth_cm > 30 else "processing" if depth_cm > 15 else "optimal"
            status_text = f"Cảnh báo: Ngập {depth_cm}cm" if depth_cm > 15 else "Khô ráo • An toàn"
            coords = spot.get("segment_coords")
            road_corridor = {"type": "LineString", "coordinates": coords} if coords else None

            locations.append({
                "id": f"flood-{spot['id']}",
                "name": f"Điểm ngập {spot['name']}",
                "category": "flood",
                "district": spot["district"],
                "address": f"{spot['street']}, {spot['district']}",
                "latitude": spot["lat"],
                "longitude": spot["lng"],
                ...
                "metricValue": f"{depth_cm} cm ({severity})",
                "severityLevel": severity,
                "causeType": spot["cause"],
                "causeDesc": spot["cause"],
                "riskScore": 65.0 if depth_cm > 25 else 20.0,
                "isImpassableBikes": depth_cm > 30,
                "isImpassableCars": depth_cm > 45,
                "description": f"{spot['detour_advice']} (Trạm bơm: {spot.get('pump_station', 'N/A')}).",
            })
